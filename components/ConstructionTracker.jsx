...
function Select({ value, onChange, options }) { return <select value={value} onChange={e => onChange(e.target.value)} style={{ padding: "8px 10px", borderRadius: 12, border: "1px solid #d1d5db", width: "100%" }}>{options.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select>; }
...
export default function ConstructionTracker() {
  ...
  const [form, setForm] = useState({
    name: "", activity: "excav_soft", quantity: "", durationDays: "",
    skilledPct: 60, equipment: "Excavator", unitCost: "",
    predecessor: "", material: "", materialQty: "", materialLead: 3
  });
  ...
  const calc = t => {
    ...
    const workerDays = qty / prod, manpower = dur ? workerDays / dur : 0;
    const skilled = (manpower * t.skilledPct) / 100, unskilled = manpower - skilled;
  ...
  const procurement = scheduled.filter(t => t.material).map(t => { const d = new Date(t.start); d.setDate(d.getDate() - Number(t.materialLead || 0)); return { task: t.name, material: t.material, qty: t.materialQty, orderBy: d.toISOString().slice(0, 10) }; });
      ...
      <Card><CardContent style={{ overflowX: "auto", padding: 0 }}>
        <table style={{ width: "100%", fontSize: 13 }}>
          <thead><tr style={{ background: "#f3f4f6" }}>
            <th>ID</th><th>Task</th><th>Act</th><th>Qty</th><th>Dur</th><th>Start</th><th>Finish</th><th>Wk-days</th><th>Manp</th><th>Cost</th><th></th>
          </tr></thead>
          <tbody>
          {scheduled.map(t => { const c = calc(t); const isCP = cp.set.has(t.id);
            return (<tr key={t.id} style={{ background: isCP ? "#fee2e2" : "transparent", borderTop: "1px solid #e5e7eb" }}>
              <td style={{ padding: 8 }}>{t.id.slice(0, 5)}</td><td>{t.name}</td><td>{t.activity}</td>
              <td style={{ textAlign: "right" }}>{t.quantity}</td>
              <td style={{ textAlign: "right" }}>{t.durationDays}</td>
              <td>{t.start}</td><td>{t.finish}</td>
              <td style={{ textAlign: "right" }}>{c.workerDays}</td>
              <td style={{ textAlign: "right" }}>{c.manpowerAvg}</td>
              <td style={{ textAlign: "right" }}>₹{c.cost}</td>
